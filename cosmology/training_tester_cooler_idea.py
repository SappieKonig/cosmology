...
import numpy as np
from cosmology.weird_epic_new_model import make_epic_model
import tqdm
...
ran = np.tile(np.arange(d_small), d_small)
var1 = ran.reshape((d_small, d_small), order="F").flatten()
var2 = ran.reshape((d_small, d_small), order="C").flatten()
options1 = np.array([var1 + i for i in range(size - 2 * r)])
options2 = np.array([var2 + i for i in range(size - 2 * r)])

model = make_epic_model(d_big, d_small, 1)
...
save_path = "C:/tensorTestModels/please_just_work_for_once_2D_v7"
# model.load_weights(save_path)

"""
It is more efficient to first preprocess everything, only to grab it from RAM after.
...
def process(choices):
    parts = indices_grid[options1[choices[:, 0] - d_small//2], options2[choices[:, 1] - d_small//2]].reshape(-1,
                                                                                                       d_small,
                                                                                                       d_small,
                                                                                                       depth)
    ...
    inputs = []
    for part in parts:
        _in = in_n_out_from_record(part)
        inputs += [_in]
    inputs = np.array(inputs)
    return inputs

def in_n_out_from_record(shard):
    uniques = pos_kappa_mass_by_index[shard[shard != -1]]
    importance = uniques[:, 4]
    candidats = np.argsort(importance[::-1])[:input_len]
    winners = uniques[candidats]
    input_part = winners[:, [0, 1, 2, 4]]
    padded = np.zeros((input_len, 4))
    padded[:len(input_part)] += input_part
    return padded

axis_1 = ra_max + d_small * (d_big//2) + 1 - (ra_min - d_small * (d_big//2))
axis_2 = dec_max + d_small * (d_big//2) + 1 - (dec_min - d_small * (d_big//2))

empty = np.zeros((axis_1, axis_2, input_len, 4))

arr_1 = np.arange(ra_min - d_small * (d_big//2), ra_max + d_small * (d_big//2) + 1)
arr_2 = np.arange(dec_min - d_small * (d_big//2), dec_max + d_small * (d_big//2) + 1)

new_ra_start = ra_min - d_small * (d_big//2)
new_dec_start = dec_min - d_small * (d_big//2)

np.set_printoptions(threshold=np.inf)

for index, i in enumerate(arr_1):
    flat = np.array([i]*len(arr_2))
    choices = np.stack([flat, arr_2], axis=1)
    filler = process(choices)
    print(index)
    empty[index] = filler

base = np.arange(-(d_big // 2), (d_big // 2 + 1)) * d_small

base = np.tile(base, d_big)
base_1 = base.reshape((d_big, d_big), order="F").flatten()
base_2 = base.reshape((d_big, d_big), order="C").flatten()


base_1 = np.tile(base_1, (batch_size, 1)) - new_ra_start
base_2 = np.tile(base_2, (batch_size, 1)) - new_dec_start

def get_free_shards(choices):
    x, y, z = indices[choices, 0], indices[choices, 1], indices[choices, 2]
    records = pos_kappa_mass_by_index[choices]
    shift_x, shift_y = base_1 + x.reshape((-1, 1)), base_2 + y.reshape((-1, 1))
    relative = empty[shift_x, shift_y]
    relative[:, :, :, [0, 1, 2]] -= records[:, [0, 1, 2]].reshape((-1, 1, 1, 3))
    return relative, records[:, 3].reshape((-1, 1))
...
for _ in range(10000):
    ...
    for batch in p_bar:
        ...
        net_input, net_output = get_free_shards(choices[batch * batch_size:(batch + 1) * batch_size])
        start = time.time()
