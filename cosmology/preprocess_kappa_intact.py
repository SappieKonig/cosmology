...
def get_map(index):
    ...
    counter = np.zeros((size, size, 10), dtype=np.int8)
    ...
    def map_to_place(ra_index, dec_index, z_index, data):
        ...
        count = np.zeros((size, size, 10), dtype=np.int8)
        temp_map[ra_index, dec_index, z_index] = data[i, index].astype(np.float32)
        count[ra_index, dec_index, z_index] += 1
    ...
    for i in range(500_000_000 // batch_size):
        ...
        result, count = data_to_map(file.to_numpy())
    ...
    np.save("C:/datasets/indices.npy", np.where(map!=0))
